...
from models.functions.losses import XPDNetLoss
from models.functions.nms import point_nms

import eval as eval_script

parser = argparse.ArgumentParser(description='XPDNet Training Script')
# Basic Settings
parser.add_argument('--dataset', default=None, type=str,
                    help='If specified, override the dataset specified in the config with this one.')
parser.add_argument('--config', default='XPDNet_50_config',
...
class NetLoss(nn.Module):
    ...
    def __init__(self, net:XPDNet, criterion:XPDNetLoss):
        super().__init__()
        self.net = net
    ...
    def forward(self, batched_images, batched_gt_instances, batched_gt_depths):
        ...
        Returns:
            - losses: a dict, losses from XPDNet
        """
...
def train():
    ...
    optimizer = optim.Adam([
        ...
        {'params': net.depth_estimator.parameters(), 'lr': 2*args.lr}], lr=args.lr)
    
    criterion = XPDNetLoss()
...
def log_visual_example(prn_net: XPDNet, val_dataset: PlaneAnnoDataset, writer: SummaryWriter, iteration, eval_nums=5):
    """
