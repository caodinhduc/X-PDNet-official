...
def evaluate(net: XPDNet, dataset, during_training=False, eval_nums=-1):
...
def tensorborad_visual_log(net: XPDNet, dataset, writer: SummaryWriter, iteration, eval_nums):
    dataset_indices = list(range(len(dataset)))
    random.shuffle(dataset_indices)
    dataset_indices = dataset_indices[:eval_nums]
