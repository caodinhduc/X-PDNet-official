...
class XPDNetLoss(nn.Module):
    """ Joint Weighted Loss Function for XPDNet
    Losses from PlaneRecNet:
    ...
    Proposed Loss:
        1) Depth Guided Boundary Preserving Loss
    """

    def __init__(self):
        super(XPDNetLoss, self).__init__()
    ...
    def forward(self, net, mask_preds, cate_preds, kernel_preds, depth_preds, gt_instances, gt_depths):
        ...
        gradient_masks = F.interpolate(batched_gt_scale_invariant_gradients, (120, 160), mode='bilinear', align_corners=False)
